...
export async function prepareMap(): Promise<void> {
  ...
  const features: MapFeature[] = source.elements.filter(e => e.geometry && e.geometry.length >= 2).map(e => ({ kind: e.tags?.railway === 'rail' ? 'rail' : 'river', points: e.geometry!.map(p => [p.lon, p.lat]) }));
